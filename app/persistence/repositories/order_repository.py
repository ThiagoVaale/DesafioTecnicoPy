...
class OrderRepository:
    ...
    def get_order_client(self, id_client: UUID) -> List[Order]:
        return self.session.exec(select(Order).where(Order.client_id == id_client).options(selectinload(Order.order_items)))
    
    def get_order_employee(self, id_employee: UUID) -> List[Order]:
        return self.session.exec(select(Order).where(Order.employee_id == id_employee).options(selectinload(Order.order_items)))
